...
public class PythonRefiner : CommonLanguageRefiner, ILanguageRefiner
{
    ...
    private static void AddPropertiesAndMethodTypesImportsPython(CodeElement current, bool includeParentNamespaces, bool includeCurrentNamespace, bool compareOnDeclaration) {
        Func<IEnumerable<CodeTypeBase>, IEnumerable<CodeTypeBase>> codeTypeFilter = (collection) =>
        {
            var nestedTypes = collection.Where(
            ...
            return collection.Except(nestedTypes);
        };

        AddPropertiesAndMethodTypesImports(current, includeParentNamespaces, includeCurrentNamespace, compareOnDeclaration, codeTypeFilter);
}
